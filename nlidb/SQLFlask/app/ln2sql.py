...
class ln2sql:
    ...
    def __init__(self, database_path, input_sentence, language_path, thesaurus_path, json_output_path):
        ...
        list_of_q, list_of_real_q = parser.parse_sentence(input_sentence)

        '''if json_output_path is not None:
            self.remove_json(json_output_path)
            for query in queries:
                query.print_json(json_output_path)'''
        for queries, real_queries in zip(list_of_q, list_of_real_q):
            for query in queries:
                query = str(query)
                # print query,
    ...
    def return_SQL_queries(self):
        global list_of_q
        global list_of_real_q
        return zip(list_of_q, list_of_real_q)
